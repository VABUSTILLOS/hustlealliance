  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.access_token) {
        ...
        // Save user info immediately (we have name from signup form)
        saveUserInfo({
          email,
          name: name || email.split('@')[0],
          avatar: getAvatarUrl(name, email),
        });

        // Also fetch confirmed user data from Supabase
        try {
          ...
          if (userRes.ok) {
            ...
            saveUserInfo({
              ...
              avatar: getAvatarUrl(userData.user_metadata?.full_name || name, userData.email),
            });
          }
        } catch { /* non-blocking */ }
        ...
        <GlassCard className="p-8 sm:p-10">
          ...
          <div className="text-center mb-8">
            <h1 className="gradient-text text-2xl sm:text-3xl font-heading font-bold mb-2">
              {t.signup.title || 'Join Hustle Alliance'}
            </h1>
            <p className="text-foreground-muted font-body text-sm">
              {t.signup.subtitle || 'Start your founder journey today'}
            </p>
          </div>
          ...
          {message && (
            <div className="mb-4 p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-sm font-body">
              {message}
            </div>
          )}
            ...
            <form onSubmit={handleSignup} className="space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={t.signup.namePlaceholder || 'Alex Rivera'}
                  required
              ...
              >
                {loading ? (t.signup.creating || 'Creating account...') : (t.signup.createAccount || 'Create Account')}
              </button>
            </form>
          )}
          ...
          {!message && (
            <>
              ...
              >
                ...
                {t.signup.signUpGoogle || 'Sign up with Google'}
              </button>
            </>
          )}
