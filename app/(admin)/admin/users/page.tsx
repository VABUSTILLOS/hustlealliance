...
export default function AdminUsersPage() {
  ...
  return (
    <div className="p-4 md:p-8">
      <h1 className="text-2xl font-heading font-bold text-foreground mb-2">{t.admin.users.title}</h1>
      <p className="text-muted text-sm mb-8">{t.admin.users.totalUsers.replace('{total}', String(total))}</p>
      ...
      <div className="flex gap-3 mb-6 flex-wrap">
        ...
        <select value={tierFilter} onChange={(e) => setTierFilter(e.target.value)} className="px-4 py-2 bg-surface border border-surface-light rounded-xl text-foreground text-sm focus:outline-none focus:border-accent">
          <option value="">{t.admin.users.filterAllTiers}</option>
          <option value="FREE">{tierLabels.FREE}</option>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-muted text-left border-b border-surface-light">
                ...
                <th className="p-4 font-medium">{t.admin.users.table.email}</th>
                <th className="p-4 font-medium">{t.admin.users.table.role}</th>
                <th className="p-4 font-medium">{t.admin.users.table.tier}</th>
                <th className="p-4 font-medium">{t.admin.users.table.enrollments}</th>
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u.id} className="border-b border-surface-light/50 hover:bg-surface-light/20">
                  <td className="p-4">
                    <p className="text-foreground font-medium">{u.name || t.admin.users.na}</p>
                  </td>
                  ...
                  <td className="p-4">
                    {editingUser === u.id ? (
                      <div className="flex gap-2">
                        <button onClick={() => handleSave(u.id)} className="px-3 py-1.5 text-xs rounded-lg bg-accent text-white hover:bg-accent/90">{t.admin.common.save}</button>
                        <button onClick={() => setEditingUser(null)} className="px-3 py-1.5 text-xs rounded-lg bg-surface-light text-muted hover:text-foreground">{t.admin.common.cancel}</button>
                      </div>
                    ) : (
                      <button onClick={() => openEdit(u)} className="px-3 py-1.5 text-xs rounded-lg bg-surface-light text-foreground hover:bg-accent/10 transition-colors">{t.admin.common.edit}</button>
                    )}
                  </td>
                </tr>
              ))}
