  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-8 max-w-5xl mx-auto">
      ...
      >
        ...
        <p className="text-foreground-muted text-sm leading-relaxed">{profile.startupPitch}</p>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-surface border border-surface-light rounded-2xl p-6">
            <h2 className="font-heading font-bold text-foreground text-lg mb-5">{t.profile.recentActivity}</h2>
              ...
              <div className="space-y-4">
                {memberPosts.slice(0, 5).map((post) => (
                  <div key={post.id} className="pb-4 border-b border-surface-light last:border-0 last:pb-0">
                    ...
                    <p className="text-foreground-muted text-sm">{post.text}</p>
        ...
        <div className="space-y-6">
          {/* Achievements */}
          <div className="bg-surface border border-surface-light rounded-2xl p-6">
            <h2 className="font-heading font-bold text-foreground text-lg mb-4">{t.profile.achievements}</h2>
          ...
          {completedPaths.length > 0 && (
            <div className="bg-surface border border-surface-light rounded-2xl p-6">
              <h2 className="font-heading font-bold text-foreground text-lg mb-4">{t.profile.completedPaths}</h2>
              <div className="space-y-2">
                {completedPaths.map((lp) => (
                  <Link key={lp.slug} href={`/learning/${lp.slug}`}
                    ...
                    <span className="text-foreground text-sm font-medium">{lp.title}</span>
                  </Link>
                ))}
              </div>
            </div>
          ...
          {memberSpaces.length > 0 && (
            <div className="bg-surface border border-surface-light rounded-2xl p-6">
              <h2 className="font-heading font-bold text-foreground text-lg mb-4">{t.spaces.tag}</h2>
              <div className="flex flex-wrap gap-2">
                {memberSpaces.map((s) => (
                  <Link key={s.slug} href={`/spaces/${s.slug}`}
                    className="px-3 py-1.5 rounded-full bg-surface-light border border-white/5 text-foreground text-xs hover:border-accent/30 transition-all">
                    {s.name}
                  </Link>
                ))}
              </div>
            ...
            >
              ...
              {messageSent ? (
                ...
                >
                  ...
                  <p className="font-heading font-bold text-foreground text-lg">{t.profile.messageSent}</p>
                ...
                <>
                  <textarea
                    ...
                    rows={4}
                    className="w-full bg-surface-light border border-white/10 rounded-xl px-4 py-3 text-foreground text-sm outline-none placeholder:text-muted resize-none mb-4"
                    autoFocus
                  />
                  <div className="flex justify-end gap-3">
                    ...
                    <button
                      ...
                      className="px-5 py-2 bg-accent text-foreground font-heading font-bold text-sm rounded-xl hover:bg-accent-glow disabled:opacity-40 disabled:cursor-not-allowed transition-all"
                    >
