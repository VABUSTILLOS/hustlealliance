    ...
    <div className="px-4 sm:px-6 lg:px-8 py-8 max-w-3xl mx-auto">
      ...
      {applications?.length === 0 && !isLoading && (
        <div className="text-center py-12 bg-white rounded-2xl border">
          ...
          <p className="text-gray-500 mt-1 mb-4">Start applying to jobs on the board</p>
          <Link
            href="/jobs"
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="text-sm text-gray-500">
                    {(app.job as { company?: string })?.company || "Unknown Company"}
                  </p>
                </div>
                <ApplicationStatusBadge status={app.status as Parameters<typeof ApplicationStatusBadge>[0]["status"]} />
              </div>
              <div className="mt-2 flex items-center gap-4 text-xs text-gray-400">
                <span>Applied {new Date(app.createdAt as string).toLocaleDateString()}</span>
                {(app.coverLetter as string) && <span>Cover letter included</span>}
              </div>
            </Link>
          ))}
        </div>
